...
async function main() {
  ...
  await Bluebird.map(
    ...
    async (order) => {
      const idx = ++index
      const log = (...message) =>
        console.log(`[${idx}/${total}]\t[${order.from}-${order.to}]\t${order.orderId}\t`, ...message)

      const fromClient = await order.fromClient()
      const toClient = await order.toClient()

      order.fromRateUsd = BN(order.fromUsdValue)
        .div(unitToCurrency(assets[order.from], order.fromAmount))
        .dp(2)
        .toNumber()
